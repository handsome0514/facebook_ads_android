            ...
            public void onClick(View view) {
                try {
                    if(reason.getText().toString().equals("")) {
                        makeText(getApplicationContext(), R.string.messagefield, Toast.LENGTH_LONG).show();
                    } else {
...
//            }
//        }
//        super.onRequestPermissionsRes ult(requestCode, permissions, grantResults);
//    }
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == 100) {
            if (grantResults.length > 0) {
                if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    // Permission granted, proceed with necessary actions
                } else {
                    requestPermissions(new String[]{Manifest.permission.READ_EXTERNAL_STORAGE}, 100);
                }
            } else {
                            ...
                            try {
                                ...
                                if(Jarray.equals("1")){
                                    runOnUiThread(new Runnable(){
                                        public void run() {
                                            makeText(mContext, getApplicationContext().getString(R.string.submit_success), Toast.LENGTH_SHORT).show();
                                            finish();
                                        }
                                    });
                                }else{
                                    runOnUiThread(new Runnable(){
                                        public void run() {
                                            try {
                                                ...
                                                makeText(mContext, error.getString("reason"), Toast.LENGTH_SHORT).show();
                                ...
                                if(Jarray.equals("1")){
                                    ...
                                    runOnUiThread(new Runnable(){
                                        public void run() {
                                            try {
                                                JSONObject error = Jobject.getJSONObject("error");
                                                makeText(mContext, error.getString("file"), Toast.LENGTH_SHORT).show();
                                            } catch (JSONException e) {
                                                e.printStackTrace();
                                            }
